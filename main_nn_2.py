    ...
    for m in range(1,3):
        ...
        for r in range(1,7): 
            ...
            if (r in [1, 3, 4, 6]):
                ...
                train = False
            for x in range(0, len(rep), 20):
                entry = rep.iloc[x:x+50, :]
...
print('------------------')
X_train = np.array(X_train).reshape(X_train.shape[0],50,10)
X_test = np.array(X_test).reshape(X_test.shape[0],50,10)
...
input = Input(shape =(50,10,1))
x = Conv2D(32,(1,10), kernel_initializer='glorot_normal', activation='relu', padding='same', input_shape = (50,10))(input)
x = Dropout(0.15)(x)
...
#    Conv2D(32,(1,10), activation='relu', padding='same'),
#    Dropout(0.15),
